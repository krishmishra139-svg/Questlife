...
export default function Dashboard() {
  ...
  const saveChar = (nc: any, currentUid?: string) => {
    const uid = currentUid || userId
    if (!uid || uid === 'guest') return
    setChar(nc)
    localStorage.setItem(`questlife_character_${uid}`, JSON.stringify(nc))
    supabase.from('leaderboard').upsert({
      ...
      xp: nc.xp,
      gold: nc.gold,
      strength: nc.attributes.strength,
      streak: nc.streak,
      updated_at: new Date().toISOString()
    }, { onConflict: 'user_id' }).then(() => {})
  }

  const saveQuests = (nq: any[], currentUid?: string) => {
    const uid = currentUid || userId
    if (!uid || uid === 'guest') return
    setQuests(nq)
    localStorage.setItem(`questlife_quests_${uid}`, JSON.stringify(nq))
  }
      ...
      quests.forEach((q: any) => {
        ...
        if (diff > 0 && diff < 5 * 60 * 1000 && !q.notified) {
          ...
          if ('Notification' in window && Notification.permission === 'granted') {
            new Notification('Quest Due Soon!', { body: msg })
          }
        ...
        if (diff < 0 && !q.penalized) {
          ...
          if ('Notification' in window && Notification.permission === 'granted') {
            new Notification('Quest Failed - Penalty!', { body: msg })
          } else {
            alert(msg)
          }
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const userEmailName = user.email ? user.email.split('@')[0] : 'Adventurer'
      const qKey = `questlife_quests_${authId}`
      const cKey = `questlife_character_${authId}`

      let q: any[] = []
      try {
        q = JSON.parse(localStorage.getItem(qKey) || '[]')
      } catch {
        q = []
      }
      ...
      if (!c || !c.name || c.name === 'Adventurer') {
        c = {
          name: userEmailName,
          bio: 'New adventurer ready to quest!',
          avatar: '',
          level: 1,
          xp: 0,
          gold: 50,
          streak: 1,
          lastStreakDate: '',
          attributes: { strength: 10, intelligence: 10, agility: 10 },
          inventory: [],
          equipped: [],
          bossDefeated: 0,
          weeklyXP: []
      ...
      if (!c.bossDefeated) c.bossDefeated = 0
      if (!c.weeklyXP) c.weeklyXP = []
      if (!c.equipped) c.equipped = []

      const today = getToday()
      if (c.lastStreakDate && c.lastStreakDate !== today && c.lastStreakDate !== getYesterday()) {
        c.streak = 1
      }
  ...
  const handlePhoto = (e: any) => {
    ...
    if (!file) return
    if (file.size > 500000) return alert('Use <500KB')
  ...
  const saveProfile = () => {
    ...
    saveChar({ ...char, name: editName.trim(), bio: editBio.trim(), avatar: editAvatar })
  ...
  const handleComplete = (id: string) => {
    ...
    if (quest.repeat === 'daily') {
      newQuests = quests.map((q: any) => q.id === id ? { ...q, lastDone: getToday(), notified: false, overdueNotified: false, penalized: false } : q)
    } else if (quest.repeat === 'weekly') {
      const next = new Date()
      next.setDate(next.getDate() + 7)
      newQuests = quests.map((q: any) => q.id === id ? { ...q, dueDate: next.toISOString().split('T')[0], notified: false, overdueNotified: false, penalized: false } : q)
    } else {
      newQuests = quests.filter((q: any) => q.id !== id)
    }
    ...
    if (char.lastStreakDate !== today) {
      if (char.lastStreakDate === yesterday || char.lastStreakDate === '') {
        newStreak = char.lastStreakDate ? char.streak + 1 : 1
      } else {
        const diffDays = Math.floor((new Date().getTime() - new Date(char.lastStreakDate).getTime()) / (1000 * 60 * 60 * 24))
        newStreak = diffDays === 1 ? char.streak + 1 : 1
      }
    }

    const newXp = char.xp + rewardXP
    const newGold = char.gold + rewardGold
    const newLevel = Math.floor(newXp / 100) + 1
    const attrs = { ...char.attributes }
    if (quest.category === 'strength') attrs.strength++
    else if (quest.category === 'intellect') attrs.intelligence++
    else attrs.agility++
    ...
    if (newBossHP <= 0) {
      newBossHP = 10
      alert('👹 BOSS DEFEATED! +200 Gold!')
      saveChar({ ...char, gold: newGold + 200 })
    }

    const oldRank = getRank(char.level).name
    const newR = getRank(newLevel)
    if (oldRank !== newR.name) alert(`🎉 RANK UP! ${oldRank} → ${newR.name} ${newR.icon}`)
  ...
  const handleDelete = (id: string) => {
    ...
    saveQuests(quests.filter((q: any) => q.id !== id))
  }

  const openEditQuest = (q: any) => {
    setEditingQuest(q)
    setEditQTitle(q.title)
    setEditQCat(q.category)
    setEditQDiff(q.difficulty || 'medium')
    setEditQIcon(q.icon || '💪')
    setEditQDue(q.dueDate || '')
    setEditQTime(q.dueTime || '')
    setEditQRepeat(q.repeat || 'none')
    setShowEditQuest(true)
  }

  const handleUpdateQuest = () => {
    if (!editQTitle.trim()) return alert('Title required')
    if (!editQDue || !editQTime) return alert('Set date and time')
    const updated = quests.map((q: any) => q.id === editingQuest.id ? {
      ...q,
  ...
  const buyItem = (id: string, cost: number) => {
    if (char.gold < cost || char.inventory.includes(id)) return
    if (id === 'potion') {
      saveChar({ ...char, gold: char.gold - cost, xp: char.xp + 100, level: Math.floor((char.xp + 100) / 100) + 1 })
      return
    }
    saveChar({ ...char, gold: char.gold - cost, inventory: [...char.inventory, id] })
  }

  const toggleEquip = (id: string) => {
    const isEq = char.equipped?.includes(id)
    let ne, na = { ...char.attributes } as any
    if (isEq) {
      ne = char.equipped.filter((x: string) => x !== id)
      if (id === 'sword') na.strength -= 5
      if (id === 'shield') na.agility -= 5
      if (id === 'crown') na.intelligence -= 10
    } else {
      ne = [...(char.equipped || []), id]
      if (id === 'sword') na.strength += 5
      if (id === 'shield') na.agility += 5
    ...
    saveChar({ ...char, equipped: ne, attributes: na })
  }

  const useItem = (id: string) => {
    if (id === 'gem') {
      if (!confirm('Use Gem +50 Gold?')) return
      saveChar({ ...char, gold: char.gold + 50, inventory: char.inventory.filter((x: string) => x !== id), equipped: char.equipped.filter((x: string) => x !== id) })
    } else if (id === 'potion') {
      saveChar({ ...char, xp: char.xp + 100, level: Math.floor((char.xp + 100) / 100) + 1, inventory: char.inventory.filter((x: string) => x !== id) })
    } else {
      toggleEquip(id)
    }
  }
  ...
  const nextRank = getNextRank(char.level)
        ...
        <div className="max-w-[1300px] mx-auto">
          <div className="flex justify-between items-center mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-black tracking-widest leading-none">QUESTLIFE</h1>
                <p className="text-[10px] text-white/50">{char.name}</p>
              </div>
            </div>
            <div className="flex gap-2 items-center">
              <div className={`px-3 py-1.5 rounded-full font-black text-xs ${rank.color} ${rank.text}`}>{rank.icon} {rank.name}</div>
              <div className="px-3 py-1.5 rounded-full bg-amber-400/20 text-amber-300 font-bold text-xs">{char.gold} G • {char.xp} XP</div>
              <div className="px-3 py-1.5 rounded-full bg-red-500/20 text-red-300 font-bold text-xs">👹 HP:{bossHP}/10</div>
              <button 
                onClick={async () => {
                ...
                className="px-3 py-1.5 rounded-full bg-white/10 hover:bg-white/20 text-xs font-bold transition-all"
              >
                Sign out
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <div className="space-y-5">
              <div onClick={openProfile} className={`rounded-[20px] bg-[#1e3a5f]/85 backdrop-blur-md p-5 cursor-pointer border-2 ${rank.border} hover:ring-2 hover:ring-yellow-400/50`}>
                <div className="flex gap-3">
                  <div className="relative">
                    {char.avatar ? (
                      <img src={char.avatar} className={`w-16 h-16 rounded-xl object-cover border-2 ${rank.border}`} />
                    ) : (
                      <div className={`w-16 h-16 rounded-xl flex items-center justify-center text-2xl border-2 ${rank.border} bg-yellow-400`}>🧑</div>
                    )}
                    <div className={`absolute -bottom-1 -right-1 w-6 h-6 ${rank.color} rounded-full flex items-center justify-center text-xs border border-white`}>{rank.icon}</div>
                  </div>
                  <div className="flex-1">
                    <h2 className="font-black text-lg">{char.name} <span className="text-[9px] bg-white/20 px-1.5 py-0.5 rounded-full">EDIT</span></h2>
                    <div className={`inline-block mt-1 px-2 py-0.5 rounded-full text-[9px] font-black ${rank.color} ${rank.text}`}>{rank.name} • Lv.{char.level}</div>
                    <p className="text-white/50 text-xs mt-1 truncate">{char.bio}</p>
                    <div className="mt-1 h-1.5 bg-black/30 rounded-full overflow-hidden w-28">
                      <div className="h-full bg-cyan-400" style={{ width: `${char.xp % 100}%` }} />
                    </div>
                  </div>
                </div>
                <div className="mt-3 rounded-xl bg-black/30 p-2 border border-white/10">
                  <div className="flex justify-between text-[9px] text-white/50">
                    <span>{rank.icon} {rank.name}</span>
                    <span>{nextRank ? `${nextRank.icon} ${nextRank.name} Lv.${nextRank.min}` : 'MAX'}</span>
                  </div>
                  <div className="mt-1 h-1.5 bg-black/40 rounded-full overflow-hidden">
                    <div className={`h-full ${rank.color}`} style={{ width: nextRank ? `${((char.level - rank.min) / (nextRank.min - rank.min)) * 100}%` : '100%' }} />
                  </div>
                </div>
                <div className="mt-3 flex gap-1.5 text-[11px] font-bold">
                  <span className="px-2 py-1 rounded bg-black/20">STR:{char.attributes.strength}</span>
                  <span className="px-2 py-1 rounded bg-black/20">INT:{char.attributes.intelligence}</span>
                  <span className="px-2 py-1 rounded bg-black/20">AGI:{char.attributes.agility}</span>
                </div>
                <div className="mt-3 rounded-xl bg-black/20 p-2.5">
                  <p className="text-[10px] text-white/40">STREAK 🔥 {char.streak} {char.lastStreakDate === getToday() && '✓'}</p>
                  <p className="text-[11px] mt-1">{char.lastStreakDate === getToday() ? 'Saved today!' : 'Do 1 quest!'}</p>
                </div>
              </div>

              <div className="rounded-[20px] bg-white/[0.07] backdrop-blur-md border border-white/10 p-4">
                <h3 className="font-black text-[11px] tracking-widest mb-2">👹 WEEKLY BOSS</h3>
                <div className="h-3 bg-black/40 rounded-full overflow-hidden">
                  <div className="h-full bg-red-500 transition-all" style={{ width: `${(bossHP / 10) * 100}%` }} />
                </div>
                <p className="text-[11px] text-white/50 mt-2">{bossHP > 0 ? `Defeat ${bossHP} more quests!` : 'Boss Defeated!'} • {char.bossDefeated} defeated</p>
              </div>

              <div className="rounded-[20px] bg-white/[0.07] backdrop-blur-md border border-white/10 p-4">
                <h3 className="font-black text-[11px] tracking-widest mb-2">🏆 ACHIEVEMENTS {unlockedAch.length}/{ACHIEVEMENTS.length}</h3>
                <div className="grid grid-cols-3 gap-2">
                  {ACHIEVEMENTS.map(a => {
                    const unlocked = a.check(char, quests)
                    return (
                      <div key={a.id} className={`p-2 rounded-xl text-center border ${unlocked ? 'bg-yellow-400/20 border-yellow-400/30' : 'bg-black/20 border-white/5 opacity-40'}`}>
              ...
              <div className="rounded-[20px] bg-white/[0.07] backdrop-blur-md border border-yellow-400/20 p-4">
                <h3 className="font-black text-xs tracking-widest mb-2">🏆 LEADERBOARD</h3>
                <div className="space-y-1.5">
                  {board.map((p: any, i: number) => {
                    const pr = getRank(p.level)
                    return (
                      <div key={p.user_id} className={`flex justify-between items-center p-2 rounded-xl text-xs ${p.user_id === userId ? 'bg-yellow-400/20 border border-yellow-400/30' : 'bg-black/20'}`}>
                        <span className="font-bold truncate">{i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `#${i + 1}`} {pr.icon} {p.username}</span>
                        <span className="text-yellow-300 font-bold">{pr.name}</span>
                      </div>
                    )
                  })}
                </div>
            ...
            <div className="lg:col-span-2">
              <div className="rounded-[20px] bg-white/[0.07] backdrop-blur-md border border-white/10 p-5">
                <div className="flex justify-between items-center mb-3">
                  <h2 className="font-black text-lg">Active Quests ({filtered.length})</h2>
                  <button onClick={() => setShowAdd(true)} className="px-4 py-1.5 rounded-full bg-yellow-400 text-black font-black text-xs">+ Add Quest</button>
                </div>
                <div className="flex gap-2 mb-4 overflow-x-auto">
                  {['all', 'strength', 'intellect', 'spirit'].map(f => (
                    <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1 rounded-full text-xs font-bold capitalize whitespace-nowrap ${filter === f ? 'bg-white text-black' : 'bg-white/10 text-white/60'}`}>{f}</button>
                  ))}
                </div>
                <div className="space-y-2">
                  {filtered.map((q: any) => {
                    const d = DIFFICULTY[q.difficulty as keyof typeof DIFFICULTY] || DIFFICULTY.medium
                    const timeLeft = getTimeLeft(q)
                    const penalized = q.penalized
                    return (
                        ...
                        <div className="flex gap-2 items-center">
                          ...
                          <div>
                            <p className="font-bold text-sm flex gap-2 items-center">
                              {q.title} <span className={`text-[9px] px-1.5 py-0.5 rounded-full ${d.color}`}>{d.label}</span> 
                              {penalized && <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-red-500/30 text-red-300">PENALTY -{d.xp}XP -{d.gold}G</span>}
                            </p>
                            <p className="text-[11px] text-white/40">{q.category} • +{d.xp} XP +{d.gold} G • 📅 {q.dueDate} ⏰ {q.dueTime}</p>
                            {timeLeft && <p className={`text-[11px] ${timeLeft.color}`}>⏳ {timeLeft.text}</p>}
                          </div>
                        </div>
                        <div className="flex gap-1">
                          <button onClick={() => openEditQuest(q)} className="px-2.5 py-1.5 rounded-xl bg-white/10 text-white text-xs">✏️</button>
                          <button onClick={() => handleDelete(q.id)} className="px-2.5 py-1.5 rounded-xl bg-red-500/20 text-red-300 text-xs">🗑️</button>
                          <button onClick={() => handleComplete(q.id)} className={`px-3 py-1.5 rounded-xl font-black text-xs ${penalized ? 'bg-yellow-400 text-black' : 'bg-emerald-400 text-black'}`}>
                            {penalized ? 'Late Done' : 'Done'}
                          </button>
                        </div>
                      </div>
                    )
              ...
              <div className="mt-5 rounded-[20px] bg-white/[0.07] backdrop-blur-md border border-white/10 p-4">
                <p className="text-[10px] tracking-widest text-white/30 font-bold mb-3">ITEM SHOP + INVENTORY</p>
                <div className="grid grid-cols-1 gap-2">
                  {SHOP.map(item => {
                    const owned = char.inventory.includes(item.id)
                    const equipped = char.equipped?.includes(item.id)
                    return (
                      <div key={item.id} className={`flex justify-between items-center p-2.5 rounded-xl border ${equipped ? 'bg-yellow-400/20 border-yellow-400/40' : 'bg-black/30 border-white/10'}`}>
                        <div className="flex items-center gap-2">
                          <span className="text-lg">{item.icon}</span>
                          <div>
                            <span className="text-xs font-bold block">{item.name} {equipped && '✓'}</span>
                            <span className="text-[9px] text-white/40">{item.effect}</span>
                          </div>
                        </div>
                        {!owned ? (
                          <button onClick={() => buyItem(item.id, item.cost)} className="text-xs px-2.5 py-1 rounded-full font-bold bg-yellow-400 text-black">{item.cost} G</button>
                        ) : item.id === 'gem' || item.id === 'potion' ? (
                          <button onClick={() => useItem(item.id)} className="text-xs px-2.5 py-1 rounded-full font-bold bg-purple-500 text-white">USE</button>
                        ) : (
                          <button onClick={() => useItem(item.id)} className={`text-xs px-2.5 py-1 rounded-full font-bold ${equipped ? 'bg-white/20' : 'bg-emerald-400 text-black'}`}>
            ...
            <div className="rounded-[20px] bg-[#1e3a5f]/85 backdrop-blur-md border border-cyan-400/20 p-5">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center p-3 rounded-xl bg-black/20">
                  <span className="text-xs">Overdue Quests</span>
                  <span className="text-xs font-black text-red-400">
                    {quests.filter((q: any) => {
                      if (!q.dueDate || !q.dueTime) return false
                      return new Date(`${q.dueDate}T${q.dueTime}`).getTime() < now.getTime()
                    }).length}
                  </span>
                </div>
                <div className="flex justify-between items-center p-3 rounded-xl bg-black/20">
                  <span className="text-xs">Due Today</span>
                  <span className="text-xs font-black text-yellow-300">{quests.filter((q: any) => q.dueDate === getToday()).length}</span>
                </div>
                <div className="flex justify-between items-center p-3 rounded-xl bg-black/20">
                  <span className="text-xs">Completed Today</span>
                  <span className="text-xs font-black text-emerald-400">{char.weeklyXP?.filter((w: any) => w.date === getToday()).length || 0}</span>
                </div>
                <div className="mt-2 h-2 bg-black/30 rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-cyan-400 to-yellow-400 transition-all" style={{ width: `${Math.min(100, quests.filter((q: any) => q.dueDate === getToday()).length > 0 ? (char.weeklyXP?.filter((w: any) => w.date === getToday()).length / quests.filter((q: any) => q.dueDate === getToday()).length) * 100 : 0)}%` }} />
                </div>
